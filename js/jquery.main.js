...
function initStartAnimation(){
	var animClass = 'go-anim';
	var busyClass = 'busy';
	var delay = 400;
	var lineBoxes = 'line-anim';
	var orderBoxes = 'order-anim';
	var win = jQuery(window);
	jQuery('.anim-box').each(function(){
		var box = jQuery(this);
		var parent = box.parent();
		if(parent.hasClass(lineBoxes) || parent.hasClass(orderBoxes)){
			var queue = [];
			var n = 0;
			var busy = false;
			var order = false;
			if(parent.hasClass(orderBoxes)) order = true;
			
			function searchBoxes(){
				var boxes = (!order ? parent.children() : parent.parent().find('.' + orderBoxes).children()).not('.' + busyClass).filter(function(){
					return jQuery(this).offset().top < win.scrollTop() + win.height();
				});
				if(boxes.length){
					boxes.addClass(busyClass).each(function(){
			...
			function goAnimLine(){
				if(queue[n]){
					busy = true;
					queue[n].addClass(animClass);
					setTimeout(function(){
						n++;
						if(n < queue.length){goAnimLine();}
						else busy = false;
					},delay)
				}
			}
			searchBoxes();
			win.bind('load scroll resize orientationchange',searchBoxes);
		}
		else{
			function goAnim(){
				if(box.offset().top < win.scrollTop() + win.height()) {
					box.addClass(busyClass).addClass(animClass);
				}
			}
			goAnim();
			win.bind('load scroll resize orientationchange',goAnim);
		}
	})
}
...
// Detect rules
PlatformDetect.addRule({type: 'android', version: 2, css: 'android2.css'});
